...
**Note**: All cURL examples below use the local development server (`http://localhost:8000`). To test against the testing server, replace the base URL with `https://ecv1-api.testingelmo.com` (note: no `/api/v1` at the end) and keep the full paths as they are.
