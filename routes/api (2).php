...
Route::prefix('v2/')
    ...
    ->group(function () {
        ...
        Route::prefix('admin')->group(function () {
            ...
            Route::put('brands/{id}/restore', [V2BrandController::class, 'restore'])->name('v2.brands.restore');
            Route::delete('brands/{id}/force-delete', [V2BrandController::class, 'forceDelete'])->name('v2.brands.force-delete');

            // ---------- CATEGORY MANAGEMENT ----------
            Route::apiResource('categories', V2CategoryController::class)->names('v2.categories');

            // ---------- PRODUCT MANAGEMENT ----------
            Route::apiResource('products', V2ProductController::class)->names('v2.products');

            // ---------- PRODUCT MEDIA MANAGEMENT ----------
            Route::get('products/{product}/media', [V2ProductMediaController::class, 'index'])->name('v2.products.media.index');
            Route::post('products/{product}/media', [V2ProductMediaController::class, 'store'])->name('v2.products.media.store');
            Route::delete('products/{product}/media/{media}', [V2ProductMediaController::class, 'destroy'])->name('v2.products.media.destroy');
            Route::put('products/{product}/media/{media}/set-main', V2SetProductMediaAsMainController::class)->name('v2.products.media.set-main');

            // ---------- CLIENT MANAGEMENT ----------
            Route::apiResource('clients', V2ClientController::class)->names('v2.clients');

            // ---------- ORDER MANAGEMENT ----------
            Route::apiResource('orders', V2OrderController::class)->names('v2.orders');
            Route::put('orders/{order}/approve', [V2OrderController::class, 'approve'])->name('v2.orders.approve');
            Route::put('orders/{order}/reject', [V2OrderController::class, 'reject'])->name('v2.orders.reject');
            Route::put('orders/{order}/complete', [V2OrderController::class, 'complete'])->name('v2.orders.complete');
            Route::get('orders-statistics', [V2OrderController::class, 'statistics'])->name('v2.orders.statistics');
            Route::get('orders-status-counts', [V2OrderController::class, 'statusCounts'])->name('v2.orders.status-counts');
